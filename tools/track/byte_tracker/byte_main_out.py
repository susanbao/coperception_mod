...
if __name__ == "__main__":
    ...
    for current_agent in range(args.from_agent, args.to_agent):
        ...
        if args.output_cov and args.nll_ass:
            save_path = f"./val/cfk_nlla/{args.mode}/tracking{current_agent}"
        elif args.output_cov:
            save_path = f"./val/cfk/{args.mode}/tracking{current_agent}"
        elif args.nll_ass:
            save_path = f"./val/nlla/{args.mode}/tracking{current_agent}"
        else:
            save_path = f"./val/origin/{args.mode}/tracking{current_agent}"
            ...
            with open(os.path.join(save_path, seq.replace("det_", "")), "w") as out_file:
                ...
                for frame in range(int(seq_dets[:, 0].max())):
                    ...
                    for track in trackers:
                        ...
                        cov = track.covrh
                        print(
                            "%d,%d,%.6f,%.6f,%.6f,%.6f,1,-1,-1,-1,%.6f,%.6f,%.6f,%.6f"
                            % (frame, track_id, tlwh[0], tlwh[1], tlwh[2], tlwh[3], cov[0],cov[1],cov[2],cov[3]),
                            file=out_file,
                        )
            matched_num_step, unmatched_det_step, unmatched_trk_step, improved_matched_by_nll_step = mot_tracker.return_match_result()
            matched_num += matched_num_step
            unmatched_det += unmatched_det_step
            unmatched_trk += unmatched_trk_step
            improved_matched_by_nll += improved_matched_by_nll_step
        ...
        print("matched count: {}, unmatched detection: {}, unmatched prediction: {}, improved_matched_by_nll: {}".format(matched_num, unmatched_det, unmatched_trk,improved_matched_by_nll))